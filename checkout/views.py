            ...
            for product_id, product_data in basket.items():
                try:
                    ...
                    if isinstance(product_data, int):
                        ...
                        regular_stock = get_object_or_404(
                            RegularStock, product=product)
                        if (regular_stock.stock - product_data) >= 0:
                            updated_stock = (
                                regular_stock.stock - product_data)
                            setattr(regular_stock, "stock", updated_stock)
                            regular_stock.save()
                        else:
                            messages.error(request, (
                                f"Unfortunately we only have \
                                {regular_stock.stock} left of the {product}. "
                        ...
                        for size, quantity in product_data[
                            ...
                            if (size_stock - quantity) >= 0:
                                ...
                                messages.error(request, (
                                    f"Unfortunately we only have {size_stock} \
                                    left of the {product} in {size}. Please \
                                    update the quantity, choose another size, \
                                    or remove the item from the basket to \
                                    complete your order!")
                                )
                                return redirect(reverse("view_basket"))
                    ...
                    messages.error(request, (
                        f"One of the products in your basket wasn't found \
...
def checkout_success(request, order_number):
    ...
    if request.user.is_authenticated:
        ...
        if save_info:
            ...
            user_profile_form = UserDeliveryForm(
                delivery_data,
                instance=profile
                )
    ...
    messages.success(request, f"Thank you, your order has been \
        successfully processed! Your order number is {order_number}. \
        A confirmation email will be sent to {order.email}.")

    email_subject = f"Order Confirmation - {order.order_number}"
    html_message = render_to_string(
        "checkout/confirmation_emails/order-confirmation.html", {
            "order": order,
        })
