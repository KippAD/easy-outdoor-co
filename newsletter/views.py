    ...
    if request.method == "POST":
        ...
        if email.strip() and name.strip():
            if MailingList.objects.filter(email=email).exists():
                ...
                email_content = f"Hi {name}! This is a confirmation email \
                    for your recent subscription to the Easy Outdoor Co. \
            ...
            messages.error(
                request,
                "An error occured: Please fill out both fields."
                )
